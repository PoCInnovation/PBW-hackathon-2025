...
type PredictionOutcome = 'Yes' | 'No' | string;
...
const StepperSwap = ({onStateChange}: StepperSwapProps) => {
    ...
    const [selectedOutcome, setSelectedOutcome] = useState<PredictionOutcome>('Yes');
    ...
    useEffect(() => {
      ...
      }, [currentStep, selectedMarket, fromToken, targetToken, thresholdsEnabled, positiveThreshold, selectedOutcome, onStateChange]);
                    ...
                    {market.trend}
                            ...
                            <div className="mb-3">
                                ...
                                <div className="flex justify-between mt-1">
                                    <span
                                        className="text-sm text-textSecondary">Current: {market.currentProbability}%</span>
                                    <span
                                        className="text-sm text-textSecondary">Exp: {market.expiry.toLocaleDateString()}</span>
                                </div>
                            </div>
    ...
    const renderStepTwo = () => (
        <div className="step-content">
            ...
            {/* Outcome Selection */}
            <div className="mb-4">
                <label className="block text-textSecondary mb-2">Prediction Outcome</label>
                <div className="grid grid-cols-2 gap-2">
                    ...
                    {/* Show specific outcomes if market has them */}
                    {selectedMarket?.options && selectedMarket.options.length > 2 && (
                        <div className="col-span-2 mt-2">
                            <select
                                value={selectedOutcome !== 'Yes' && selectedOutcome !== 'No' ? selectedOutcome : ''}
                                onChange={(e) => setSelectedOutcome(e.target.value)}
                                className="bg-primary border border-border rounded-lg p-2 w-full text-white focus:outline-none"
                            >
                                <option value="" disabled>Select specific outcome</option>
                                {selectedMarket.options.map((option: string) => (
            ...
            <div className="mb-6">
                <div className="flex justify-between mb-2">
                    ...
                    <div className="flex items-center">
                        <span className="mr-2 text-sm text-textSecondary">
                            {thresholdsEnabled ? 'Enabled' : 'Disabled'}
                        </span>
                        <button
                            onClick={() => setThresholdsEnabled(prev => !prev)}
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${thresholdsEnabled ? 'bg-accent' : 'bg-primary border border-border'}`}
                        >
                            <span
                                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${thresholdsEnabled ? 'translate-x-6' : 'translate-x-1'}`}
                            />
                        </button>
                    </div>
                </div>
                ...
                {thresholdsEnabled ? (
                    <>
                        <div className="flex justify-between mb-2">
                            <div>
                                <span className="text-success mr-2">Positive Threshold:</span>
                                <span>{positiveThreshold}%</span>
                            </div>
                            <div>
                                <span className="text-textSecondary mr-1">Outcome:</span>
                                <span className={selectedOutcome === 'Yes' ? 'text-success' : selectedOutcome === 'No' ? 'text-danger' : 'text-accent'}>
                                    {selectedOutcome}
                                </span>
                            </div>
                        </div>
                    ...
                    <p className="text-sm text-textSecondary mb-4">
                        Enable threshold to automatically swap tokens when the prediction probability crosses your defined value.
                    </p>
                )}
            </div>

            {/* Target Token section - conditional based on thresholds */}
            {thresholdsEnabled ? (
                <div className="mb-6">
                    <label className="flex justify-between text-textSecondary mb-2">
                        <span>Target Token (If {selectedOutcome} prediction &gt; {positiveThreshold}%)</span>
                        <span className="text-success">Swap Target</span>
                    </label>
                    <div 
                ...
                <div className="mb-6">
                    <label className="text-textSecondary mb-2">Target Token</label>
                    <div 
                        className="flex items-center justify-between bg-primary rounded-lg p-3 border border-border cursor-pointer"
                        onClick={() => setIsTokenSelectOpen('target')}
                    >
            ...
            <div className="flex space-x-3">
                ...
                <button className="btn btn-primary flex-1">
                    {thresholdsEnabled ? 'Create Threshold Position' : 'Create Direct Swap'}
                </button>
            </div>
        </div>
    );
